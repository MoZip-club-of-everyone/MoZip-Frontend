...
export default function LongAnswer({ question, details, isRequired, onSave }: { question: string; details: string; isRequired: boolean, onSave: (answer: string) => void; }) {
	...
	return (
		<CustomBox $width="100%" $height="auto" $backgroundcolor="white" $padding="1rem" $overflowx="hidden" $overflowy="hidden"
			...
			<Wrapper>
				...
				<span>{details}</span>
				<Textarea required={isRequired} rows={5} value={answer} onChange={(e) => setAnswer(e.target.value)} onBlur={handleBlur} />
			</Wrapper>
		</CustomBox>
	);
}
